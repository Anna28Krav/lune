...
// v84: loader only on selected pages and first site opening
(function(){
  const loaderPages = ['index.html', 'booking.html', 'charity.html', 'certificate.html'];
  ...
  function currentPageName(){
    return getPageNameFromUrl(window.location.pathname);
  }

  function isLoaderPage(page){
    return loaderPages.includes(page);
  }
  ...
  function createLoader(){
    ...
    preloader.innerHTML = `
      ...
      <div class="preloader-stars"><span></span><span></span><span></span><span></span><span></span></div>
      <div class="preloader-box">
        <div class="preloader-moon-wrap"><img src="images/moon-icon.svg" alt="Luné moon"></div>
        <h2>LUNÉ BEAUTY</h2>
        <p>ПРОСТІР КРАСИ</p>
  ...
  function removeLoaderSlow(){
    const preloader = document.querySelector('.preloader');
    if(preloader){
      setTimeout(()=>preloader.classList.add('hide'), 4300);
      setTimeout(()=>preloader.remove(), 5600);
    }
  }

  window.addEventListener('load', () => {
    const page = currentPageName();
    const firstSiteLoad = !sessionStorage.getItem('luneFirstLoadDone');

    if(firstSiteLoad || isLoaderPage(page)){
      sessionStorage.setItem('luneFirstLoadDone', 'true');
      createLoader();
      removeLoaderSlow();
    }else{
      removeLoaderFast();
  ...
  document.querySelectorAll('a[href]').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      const page = getPageNameFromUrl(href);
      if(!isLoaderPage(page)) return;
